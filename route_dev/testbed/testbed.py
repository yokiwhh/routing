...
def generate_request(net, src, src_port, dst, dst_port, rtype, demand, rtime, time_step): 
    ...
    for host, line in pmonitor(popens):
        ...
        if host:
            ...
            if host == dst_host:
                ...
                lossPkt = float(ret[9])
                packet = float(ret[11])
                steptime = float(ret[13])
                delaytime = float(ret[15])
                print(delay,throughput,lossPkt,packet,steptime,delaytime)
                #flag = True
    ...
    return delay, throughput, loss, (src_popen, dst_popen), lossPkt, packet, steptime, delaytime
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # rtime = data_js['rtime']
        rtime = 5
        delay, throughput, loss, popens, lossPkt, packet, steptime, delaytime= generate_request(net, data_js['src'], data_js['src_port'], data_js['dst'], data_js['dst_port'], data_js['rtype'], data_js['demand'], 1000000, time_step) # rtime is a deprecated para
        # print("%s" % (popens))
