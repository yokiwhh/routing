...
class DatanetAPI:
    ...
    def _create_routing_matrix2(self, G,routing_file):
        """
    ...
    def _create_routing_matrix(self, G,routing_file):
        ...
        # node_port_dst = self._getRoutingSrcPortDst(G)
        # R = self._readRoutingFile(routing_file, netSize)
        t = routing_file.split(';')
        MatrixPath = numpy.empty((netSize, netSize), dtype=object)
        for src in range (0,netSize):
            for dst in range (0,netSize):
                MatrixPath[src][dst] = list(t)
        '''
        #print(MatrixPath)
        return (MatrixPath)
            ...
            try:
                ...
                while(True):
                    ...
                    if (s._routing_file in self._routings_dic):
                        ...
                        # routing_matrix = self._create_routing_matrix2(g, s._routing_file)
                ...
                # traceback.print_exc()
                print("1. Error in the file: %s   iteration: %d" % (file, it))
            ctr += 1
